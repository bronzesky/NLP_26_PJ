...
an ambiguous band is resolved by a one-dimensional logistic classifier on
type-token ratio---a feature orthogonal to the RoBERTa margin (an ablation
shows TTR alone carries the effect). Under the strict
fit-on-dev / predict-test-once protocol this reaches macro-F1 $0.857$,
surpassing the best single-model baseline ($0.835$). The TTR arbiter lifts the
human-like \texttt{bloomz} generator's recall from $0.10$ to $1.00$ while
holding human specificity. We pair the detector with a plagiarism-checker-style
...
fixed-detector de-AI robustness probe: a fine-tuned detector resists light or
localized paraphrase, but sustained whole-document recursive rewriting drives
its AI rate down and flips verdicts (mean drop $0.31$, $38\%$ flips over eight
documents). A negative result---a hierarchical
...
plus a one-dimensional TTR arbiter for the ambiguous band (an ablation shows
this single orthogonal feature carries the entire effect).
...
human-like \texttt{bloomz} generator's recall from $0.10$ to $1.00$
...
protocol (the AI rate is one frozen RoBERTa, measured before and after): the
detector resists light/localized paraphrase but is worn down by sustained
whole-document recursive rewriting (mean AI-rate drop $0.31$, $38\%$ verdict
flips over 8 docs); recursion is necessary and occlusion-targeting does not
beat random under recursion (Section~\ref{sec:robust}).
...
\includegraphics[width=\linewidth]{figs/fig1_operating_point.pdf}
\caption{The detector ranks well (AUROC $0.944$) but macro-F1 collapses at the
default threshold (margin$=0$); the oracle threshold is far away. The failure
is the operating point, not the representation.}
\label{fig:opp}
\end{figure}
...
in direct conflict for any single threshold (Figure~\ref{fig:overlap}).

\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{figs/fig2_margin_overlap.pdf}
\caption{Margin distributions. \texttt{bloomz} sits inside the human upper
tail (shaded ambiguous band), while clean AI separates cleanly. No single
threshold can hold both human specificity and \texttt{bloomz} recall.}
\label{fig:overlap}
\end{figure}
...
g(\text{TTR}) & t_{\text{low}}\leq m < t_{\text{high}}
\end{cases}
\end{equation}
where $g$ is a one-dimensional logistic classifier on type-token ratio alone.
We initially fit $g$ on six orthogonal hand-crafted features, but a
leave-one-out ablation (Table~\ref{tab:ablation}) shows the decision is
\emph{causally} carried by TTR: dropping it is the only change that moves the
score, and TTR alone reproduces the full result. We therefore collapse the
arbiter to a single feature---an interpretable, overfit-resistant rule
``trust the RoBERTa margin when confident, else threshold TTR.'' Both band
edges $t_{\text{low}}, t_{\text{high}}$, the TTR logistic, and the temperature
are fit on the dev split only (the deployable P1 protocol).

\paragraph{Protocols and result.} We separate effects with three readings:
\textbf{P0} (default $0.5$ threshold, no adaptation), \textbf{P1} (deployable:
fit everything on dev, predict test once), and \textbf{P2} (fit on an
in-distribution test split). Table~\ref{tab:method} shows the deployable P1
macro-F1 is $0.857$---above the strongest single-model baseline (Fusion LGBM,
$0.835$, itself test-tuned); P2 is $0.854$, essentially tied with P1 (the
single-generator dev set plus a strongly separable feature make dev-fitting
nearly lossless, so we do not treat P2 as a meaningful ceiling). Crucially the
rule keeps \texttt{bloomz} recall at $1.00$ \emph{and} human specificity at
$0.71$ at once---a combination no single threshold achieves
...
\paragraph{Ablation.} Table~\ref{tab:ablation} isolates each design choice
(all dev-fit, applied to test). Removing the orthogonal arbiter---deciding the
ambiguous band with RoBERTa itself---drops macro-F1 from $0.857$ to $0.592$:
the entire gain is the orthogonal stage, not the band partition. No single
threshold suffices, and \texttt{bloomz} recall is strongly threshold-dependent:
at a low threshold \texttt{bloomz} is easy ($0.96$ recall at margin~$0$) but
human specificity collapses to $0.32$; at the high threshold needed to protect
human specificity ($0.95$), \texttt{bloomz} recall falls to $0.10$. Even an
oracle threshold fit on test labels reaches $0.888$ only by sacrificing
\texttt{bloomz} ($0.10$) for specificity ($0.96$); the region-aware rule instead
holds both (\texttt{bloomz} $1.00$, specificity $0.71$) by routing the
overlapping band to TTR. The leave-one-out confirms the effect is causally
concentrated in TTR (dropping it is the only ablation that moves the score,
$0.857\!\rightarrow\!0.843$; TTR alone recovers $0.857$), and the arbiter is
robust to model family (logistic / shallow tree / LGBM all $\approx0.857$)
because the band sub-problem is essentially one-dimensional---supporting our
thesis that the fix is a lightweight inductive bias, not capacity.
...
\paragraph{De-AI rewriting strategies.} We compare four rewriting strategies,
all re-scored by the same fixed RoBERTa over up to six rounds on the same eight
high-AI documents: \textbf{whole-doc recursive}; \textbf{targeted recursive}
(rewrite only the top-contributing sentences located by occlusion, guided by
the document's AI-leaning features); \textbf{targeted single-round}; and
\textbf{random recursive} (rewrite randomly chosen sentences, same budget).

\paragraph{Findings.} Table~\ref{tab:robust} reports the mean over eight
documents. Three results stand out. (1) \textbf{Recursion is essential}: a
single targeted round moves the AI rate essentially zero (drop $0.000$), only
recursion erodes it. (2) \textbf{Whole-document recursive rewriting is the
strongest attack} (mean drop $0.31$, $38\%$ verdict flips), and it is more
effective even \emph{per unit of editing} ($0.34$ vs.\ $0.15$ for targeted).
(3) Surgical, occlusion-targeted rewriting does \emph{not} beat random sentence
selection here (drop $0.14$ vs.\ $0.16$): localizing the most-incriminating
sentences gives no advantage once rewriting is recursive, because each round
re-distributes the contribution mass. A fine-tuned detector is thus robust to
light or localized paraphrase but is defeated by sustained whole-document
recursive rewriting.

\begin{table}[t]\centering\small
\begin{tabular}{lcccc}
\toprule
strategy & AI-rate drop & flip & edit frac & drop/edit \\
\midrule
whole-doc, recursive & \textbf{0.31} & \textbf{0.38} & 0.87 & \textbf{0.34} \\
targeted, recursive & 0.14 & 0.12 & 0.65 & 0.15 \\
random, recursive & 0.16 & 0.25 & 0.76 & 0.19 \\
targeted, single-round & 0.00 & 0.00 & 0.10 & 0.00 \\
\bottomrule
\end{tabular}
\caption{De-AI rewriting ablation (mean over 8 high-AI docs, fixed RoBERTa,
$\leq$6 rounds). Recursion is necessary; whole-document recursive is strongest
even per edit; occlusion-targeting does not beat random under recursion.}
\label{tab:robust}
\end{table}

\paragraph{Interpretation.} Because the detector is held fixed, these drops
faithfully measure its robustness rather than gaming a secondary stage. The
single-document trajectory in Figure~\ref{fig:deai} (one favourable case,
$0.906\!\rightarrow\!0.27$) illustrates the recursive descent, but the
eight-document means show such full flips are the exception, not the rule, and
require aggressive whole-document editing (edit fraction $0.87$). Rewritten
text stays fluent and on-topic; targeting trades effectiveness for smaller
edits but, contrary to our initial single-case impression, is not the more
efficient attack.

\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{figs/fig4_deai_descent.pdf}
\caption{A favourable single-document case: targeted recursive de-AI driving
the (fixed-RoBERTa) AI rate across the $0.5$ verdict threshold. Means over
eight documents (Table~\ref{tab:robust}) show this is the exception; whole-doc
recursive rewriting is the stronger attack on average.}
\label{fig:deai}
\end{figure}
...
is leakage-free. A residual ceiling remains in human specificity ($0.71$ inside
the ambiguous band): we found this is \emph{not} a tunable artifact---sweeping
the band arbiter's decision threshold and class weight does not move it---but a
genuine \texttt{bloomz}$\leftrightarrow$human overlap in TTR space, the price of
recovering a human-like generator. The de-AI component is strictly a robustness
probe; we do not endorse using it to evade academic-integrity systems. Finally,
our study is English-only and single-domain at test time.
...
detector fixed, it resists light or localized paraphrase and is only worn down
by sustained whole-document recursive rewriting (mean AI-rate drop $0.31$),
while a hierarchical capacity extension overfits and fails.
